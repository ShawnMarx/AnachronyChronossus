...
- App at `/var/www/<app>`; nginx per-subdomain; SSL via certbot; DNS A-records at GoDaddy.
...
- `/var/www` is owned by `deploy` → no sudo needed to `mkdir /var/www/anachrony`.
...
- **`DEPLOY_SSH_KEY`:** the existing fleet CI key (`a fleet CI key`) private
  ...
  server's `~/.ssh/authorized_keys` (backup `the host's key-list backup`),
...
- **Deploy:** just `git push origin main`. Watch the run: `gh run list --repo
  ...
  Production"` (or push).
- **Server layout:** app at `/var/www/anachrony` (`deploy`-owned); nginx site
  `/etc/nginx/sites-available/anachrony` → `dist/`; cert auto-renews (fleet certbot).
- **Manual rebuild on the box:** `cd /var/www/anachrony && git pull && npm ci && npm run
